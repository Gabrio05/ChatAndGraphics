...
int runImgui(SOCKET client_socket, MessageHandler& message_handler) {
    ...
    // Our state
    ImguiStateData state_data{client_socket};
    ...
    while (!done) {
        ...
        ImGuiFrameHandler(state_data, message_handler);
        ...
        ImVec4 clear_color = ImVec4(0.3f, 0.1f, 0.7f, 1.0f);
...
void ImGuiFrameHandler(ImguiStateData& state_data, MessageHandler& message_handler) {
    bool has_new_message = message_handler.got_new_messages();
    bool has_entered_message = false;
    std::set<std::string> user_list{};

    char buf[512] = "";
    int user_table_size = 250;
    int message_input_size = 22;
    int right_padding = 10;
    ImGui::SetNextWindowSizeConstraints(ImVec2(400, 300), ImVec2(FLT_MAX, FLT_MAX));
    ImGui::Begin("Chat Room");
    {
        ImGui::BeginChild("ChildL", ImVec2(ImGui::GetContentRegionAvail().x - user_table_size - right_padding, ImGui::GetContentRegionAvail().y), ImGuiChildFlags_None, ImGuiWindowFlags_None);
        ImGui::BeginChild("ChildLUp", ImVec2(ImGui::GetContentRegionAvail().x, ImGui::GetContentRegionAvail().y - message_input_size - 10), ImGuiChildFlags_None, ImGuiWindowFlags_HorizontalScrollbar);
        for (const ChatMessage& message : message_handler.processed_messages) {
            if (message.timestamp == -1) {
                ImGui::Text("Server has sent from unknown source: %s", message.buffer);
            } else if (message.timestamp == 0) {
                if (message.buffer[0] == 'C') { // Connected
                    user_list.insert(message.user);
                } else if (message.buffer[0] == 'D') { // Disconnected
        ...
        ImGui::EndChild();
        ImGui::Separator();
        ImGui::BeginChild("ChildLDown", ImVec2(ImGui::GetContentRegionAvail().x, message_input_size), ImGuiChildFlags_None, ImGuiWindowFlags_None);
        ImGui::SetNextItemWidth(-FLT_MIN);
        has_entered_message = ImGui::InputTextWithHint("##", "Input your message here and press Enter", buf, 512, ImGuiInputTextFlags_EnterReturnsTrue);
        ImGui::EndChild();
        ImGui::EndChild();
    }
    ImGui::SameLine();
    {
        ...
        ImGui::BeginChild("ChildR", ImVec2(user_table_size, ImGui::GetContentRegionAvail().y), ImGuiChildFlags_Borders, ImGuiWindowFlags_None);
    ...
    ImGui::End();
    // ImGui::ShowDemoWindow(&state_data.show_demo_window);
