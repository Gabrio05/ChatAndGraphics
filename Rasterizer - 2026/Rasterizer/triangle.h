...
class triangle {
    ...
    float area;        // Twice the area of the triangle
...
public:
    ...
    // Get barycentric derivatives with respect to x or y. 
    // i = 0: Alpha, 1: Beta, 2: Gamma
    float getBarycentricDerivative(const bool get_x_derivative, const int i) const {
        const vec2D v1 = v[i].p;
        const vec2D v2 = v[(i + 1) % 3].p;
        return (get_x_derivative ? (v1.y - v2.y) : (v2.x - v1.x)) / area;
    }
    ...
    void draw(Renderer& renderer, Light& L, float ka, float kd) {
        ...
        int y_under = gsl::narrow_cast<int>(minV.y);
        int x_under = gsl::narrow_cast<int>(minV.x);
        int y_above = gsl::narrow<int>(ceil(maxV.y));
        int x_above = gsl::narrow<int>(ceil(maxV.x));

        float alpha_0, beta_0, gamma_0;
        getCoordinates(vec2D(gsl::narrow<float>(x_under), gsl::narrow<float>(y_under)), alpha_0, beta_0, gamma_0);
        float alpha_x = getBarycentricDerivative(true, 0);
        float alpha_y = getBarycentricDerivative(false, 0);
        float beta_x = getBarycentricDerivative(true, 1);
