...
public:
    ...
    float& operator[] (Colour c) { return gsl::at(rgb, c); }

    // Assigns the values of another colour to this one.
    // Input Variables:
    // - c: The source color
    /*void operator = (colour c) noexcept {
        r = c.r;
        g = c.g;
        b = c.b;
    }*/
    ...
    void toRGB(unsigned char& cr, unsigned char& cg, unsigned char& cb) const {
        cr = gsl::narrow_cast<unsigned char>(std::floor(r * 255));
        cg = gsl::narrow_cast<unsigned char>(std::floor(g * 255));
        cb = gsl::narrow_cast<unsigned char>(std::floor(b * 255));
    }
