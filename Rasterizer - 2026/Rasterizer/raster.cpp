...
#pragma warning(disable: 4201, justification: "Not my nameless structs.")
#include <chrono>
...
static void sceneTest() {
    ...
    std::vector<std::unique_ptr<Mesh>> scene; // Vector to store scene objects

    for (int i = -10; i <= 10; i++) {
        for (int j = -10; j <= 10; j++) {
            std::unique_ptr<Mesh> mesh = std::make_unique<Mesh>();
            *mesh = Mesh::makeSphere(1.0f, 20, 40);
            mesh->world = matrix::makeTranslation(5.0f * i - 1, 3.0f * j, -50.0f);
            scene.push_back(std::move(mesh));
            mesh = std::make_unique<Mesh>();
            *mesh = Mesh::makeCube(1.0f);
            mesh->world = matrix::makeTranslation(5.0f * i + 1, 3.0f * j, -50.0f);
            scene.push_back(std::move(mesh));
        }
    }
    ...
    float step = -0.5f;  // Step size for camera movement
...
static matrix makeRandomRotation() {
    ...
    switch (r) {
    case 0: return matrix::makeRotateX(rng.getRandomFloat(0.f, 2.0f * (float)M_PI));
    case 1: return matrix::makeRotateY(rng.getRandomFloat(0.f, 2.0f * (float)M_PI));
    case 2: return matrix::makeRotateZ(rng.getRandomFloat(0.f, 2.0f * (float)M_PI));
